...
export default function CategoryProductsPanel({ name, type, onClose }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      let query = supabase.from("products").select("id, name, price, is_active");

      if (type === "category") query = query.eq("main_category", name);
      else query = query.eq("sub_category", name);
  ...
  return (
    <div style={overlay}>
      <div style={panel}>
        <div style={header}>
          <div>
            <h3 style={title}>{type === "category" ? "Category" : "Subcategory"}: {name}</h3>
            <p style={subtitle}>
