...
//////////// above code worked before the delete options
...
//       is_primary: images.length === 0 && idx === 0,
//       position: startPosition + idx + 1,
//       variant_id: undefined, // important
//     }));
...
//     // 🔥 DELETE FROM DB ONLY IF IT EXISTS THERE
//     if (img.variant_id) {
//       await supabase
...
//     updated = updated.map((i, idx) => ({ ...i, position: idx + 1 }));
//     onChange(updated);
//   };
...
///////////////////////// above woekd before the varient delete
...
export default function VariantImagesManager({ images, onChange }: Props) {
  ...
  const addImages = (files: FileList | null) => {
    ...
    const newImages: ProductImageUI[] = Array.from(files).map((file, idx) => ({
      ...
      is_primary: images.length === 0 && idx === 0,
      position: startPosition + idx + 1,
      variant_id: undefined, // assigned on save
    }));

    onChange([...images, ...newImages]);
  };

  /* ================= DELETE IMAGE (DB + STORAGE + UI) ================= */

  const removeImage = async (img: ProductImageUI) => {
    // 1️⃣ DELETE FROM DB (ONLY IF ALREADY SAVED)
    if (img.variant_id) {
      await supabase
        .from("product_variant_images")
        .delete()
    ...
    // 3️⃣ UPDATE UI
    let updated = images.filter((i) => i.id !== img.id);
        ...
        <div style={grid}>
          {images.map((img) => (
            <div key={img.id} style={card}>
              ...
              {img.is_primary && (
                <div style={primaryBadge}>Primary</div>
              )}
